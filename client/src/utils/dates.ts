...
export const parseTimetamp = (timestamp: string, format = '') => {
    if(!timestamp) return 
...
export const fromatTimestamp = (date: Date, timestamp: string) => {
    const timestampFormat = SameDay(date, new Date()) ? 'HH:mm' : 'DD/MM/YY'
    const result = parseTimetamp(timestamp, timestampFormat)
